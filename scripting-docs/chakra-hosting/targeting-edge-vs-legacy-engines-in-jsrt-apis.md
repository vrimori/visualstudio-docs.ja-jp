---
title: "JsRT API での Edge エンジンとJsRT API のレガシ エンジン | Microsoft Docs"
ms.custom: ""
ms.date: "01/18/2017"
...
---
# JsRT API での Edge エンジンとJsRT API のレガシ エンジン
...
## Visual C\+\+ を使用した、優先されるバージョンのコンパイル  
...
```  
 ...
 ![リンカー入力ファイルとして jsrt.lib を追加](../chakra-hosting/media/js-chakra.png "JS\_Chakra\_")  
...
```  
 ...
 ![リンカー入力ファイルとして chakrart.lib を追加](../chakra-hosting/media/js-chakra-hosting.png "JS\_Chakra\_Hosting\_")  
  
 新しいアプリケーションを開始しようとしている場合は、JsRT API に対するコードの作成を開始する準備ができています。  
  
## .NET を使用した、優先されるバージョンのコンパイル  
 .NET および P\/Invoke を使用している場合、chakra.dll ではなく jscript9.dll をインポートするように JsRT API \[DllImport\] 宣言を変更する必要があります。 さらに、`JsCreateRuntime` の定義を変更して `JsRuntimeVersion` パラメーターを削除し、`JsCreateContext` および `JsStartDebugging` の定義を変更して `IDebugApplication` パラメーターを削除します。  
