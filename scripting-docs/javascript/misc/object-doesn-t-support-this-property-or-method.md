---
title: "オブジェクトではありません &#39; このプロパティまたはメソッドをサポートする t |Microsoft ドキュメント"
ms.custom: 
ms.date: 01/18/2017
ms.prod: windows-client-threshold
ms.reviewer: 
ms.suite: 
ms.technology: javascript
ms.tgt_pltfrm: 
ms.topic: article
f1_keywords: VS.WebClient.Help.SCRIPT438
dev_langs:
- JavaScript
- TypeScript
- DHTML
ms.assetid: 35f7a9b4-74fa-4ddb-b9f2-1a92345cb2ff
caps.latest.revision: "5"
author: mikejo5000
ms.author: mikejo
manager: ghogen
ms.openlocfilehash: 9c87d7d1c23682d014924c5c9b948133d8391c72
ms.sourcegitcommit: aadb9588877418b8b55a5612c1d3842d4520ca4c
ms.translationtype: MT
ms.contentlocale: ja-JP
ms.lasthandoff: 10/27/2017
---
# <a name="object-doesn39t-support-this-property-or-method"></a>オブジェクトではありません &#39; このプロパティまたはメソッドをサポートしません。
このオブジェクトにはないプロパティまたはメソッドを指定しました。 すべてのオブジェクトが、すべてのプロパティとメソッドをサポートするわけではありません。  
  
### <a name="to-correct-this-error"></a>このエラーを解決するには  
  
-   プロパティとメソッドを調べて、入力エラーがないことを確認します。  
  
-   オブジェクトのランタイム型が予想されるオブジェクト型であることを確認します。  
  
-   詳細については、オブジェクトのドキュメントを参照してください。  
  
## <a name="see-also"></a>関連項目  
 [Object オブジェクト](../../javascript/reference/object-object-javascript.md)   
 [JavaScript オブジェクト](../../javascript/reference/javascript-objects.md)