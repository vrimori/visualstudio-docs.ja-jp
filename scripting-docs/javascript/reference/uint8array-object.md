---
title: "Uint8Array オブジェクト | Microsoft Docs"
ms.custom: ""
ms.date: "01/18/2017"
...
## パラメーター  
 `uint8Array`  
 必須。  **Uint8Array** オブジェクトを代入する変数名を指定します。  
 ...
 `byteOffset`  
 省略可能。  Uint8Array の開始位置として、バッファーの先頭からのオフセットをバイトで指定します。  
...
|定数|説明|  
|--------|--------|  
|[BYTES\_PER\_ELEMENT 定数](../../javascript/reference/bytes-per-element-constant-uint8array.md)|配列の各要素のサイズ \(バイト単位\)。|  
...
## メソッド  
 次のテーブルは、`Uint8Array` オブジェクトのメソッドの一覧です。  
