...
## <a name="includejavascriptjavascriptincludesjavascript-mdmd-operators"></a>[!INCLUDE[javascript](../javascript/includes/javascript-md.md)] の演算子  
 [!INCLUDE[javascript](../javascript/includes/javascript-md.md)] の演算子を優先順位の高い順に上から並べた表を次に示します。 同じ優先順位の演算子は、左から右に評価されます。  
  
|演算子|説明|  
|--------------|-----------------|  
|」を参照してください。 [ ] ( )|フィールド アクセス、配列のインデックス付け、関数の呼び出し、および式のグループ化|  
