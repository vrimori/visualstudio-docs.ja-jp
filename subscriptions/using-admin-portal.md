...
- 組織で購入した Visual Studio サブスクリプションに応じて、**各ユーザーの登録は 1 つのサブスクリプション レベルだけにしてください**。 複数のサブスクリプション レベルが割り当てられているサブスクリプション会員がいる場合は、1 つのレベルだけが割り当てられるように会員の設定を編集してください。 
...
- PCN 全体を取得するには、主要連絡先が VLSC にサインインする必要があります (PCN を見つけるための手順がここに記載されています)。 
...
新しい Visual Studio サブスクリプションの管理ポータルに追加される必要があるときに、主要/通知連絡先が誰かわからない場合は、[VLSC](https://www.microsoft.com/Licensing/servicecenter/default.aspx) にサインインした後にこの情報を見つけることができます。 VLSC で主要/通知連絡先を見つける手順については、[この資料]「(http://www.visualstudio.com/subscriptions/support/#!articles/962-6707-how-do-i-locate-my-primary-contact "主要連絡先を確認する方法")」を参照してください。
