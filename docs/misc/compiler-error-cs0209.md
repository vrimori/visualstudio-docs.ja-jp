...
fixed ステートメントで宣言されたローカルの型は、ポインター型でなければなりません  
 ...
 [fixed ステートメント](/dotnet/csharp/language-reference/keywords/fixed-statement)で宣言する変数は、ポインターでなければなりません。 詳細については、「[unsafe コードとポインター](/dotnet/csharp/programming-guide/unsafe-code-pointers/index)」を参照してください。  
