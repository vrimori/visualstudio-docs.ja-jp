...
|`FullFrameworkFolders`|省略可能な <xref:System.String?displayProperty=fullName>`[]` 型のパラメーターです。<br /><br /> RedistList ディレクトリが含まれる一連のフォルダーを指定します。 このディレクトリは、特定のクライアント プロファイルのフレームワーク全体を表します (たとえば、%programfiles%\reference assemblies\microsoft\framework\v4.0)。|  
