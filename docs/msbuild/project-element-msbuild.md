...
|`SDK`|省略可能な属性です。 (Visual Studio 2017 以降の .NET Core プロジェクトでのみ利用できます。)<br /><br /> .proj ファイルに追加される暗黙的インポート ステートメントの作成に利用される SDK バージョン。 たとえば、`<Project Sdk="Microsoft.NET.Sdk/1.0.0-RC" />` のようにします。|  
