...
4.  UI 応答性プロファイラーの開始時に、Visual Studio ETW Collector.exe を実行するためのアクセス許可を要求するユーザー アカウント制御ウィンドウが表示される場合があります。 **[はい]**をクリックします。  
     ...
     アプリを操作して、関連するパフォーマンス シナリオをテストします。 ワークフローの詳細については、「[UI の応答性の問題の特定](#Workflow)」および「[ビジュアル スループットの問題の特定](#IsolateVisualThroughput)」を参照してください。  
...
4.  必要に応じて、[分析用のコードのマーク](#ProfileMark) を使用して、コードにユーザー マークを追加します。  
...
###  <a name="TimelineDetails"></a> タイムライン詳細の表示  
