...
3.  既定のマーカー系列に&3; つのスパンを作成して各スパンにフラグ、メッセージ、警告を&1; つずつ記述するためのコードを追加します。 静的 [EnterSpan](assetId:///EnterSpan?qualifyHint=False&autoUpgrade=True) メソッドを呼び出して、<xref:Microsoft.ConcurrencyVisualizer.Instrumentation.Span> オブジェクトを作成します。 既定の系列に書き込む場合は、<xref:Microsoft.ConcurrencyVisualizer.Instrumentation.Markers> クラスの静的な write メソッドを使用します。  
...
5.  静的 <xref:Microsoft.ConcurrencyVisualizer.Instrumentation.Markers.CreateMarkerSeries%2A> メソッドを使用して顧客マーカー系列を作成するコードを追加します。 <xref:Microsoft.ConcurrencyVisualizer.Instrumentation.MarkerSeries> クラスには、スパンを作成し、フラグ、メッセージ、警告を記述するためのメソッドが含まれます。  
