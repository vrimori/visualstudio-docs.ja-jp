...
|[\<description\> 要素](../deployment/description-element-clickonce-deployment.md)|必須。  シェルに表示する内容、およびコントロール パネルの **\[プログラムの追加と削除\]** に項目として表示される内容を作成するために使用される、アプリケーション情報を指定します。|`publisher`<br /><br /> `product`<br /><br /> `supportUrl`|  
|[\<deployment\> 要素](../deployment/deployment-element-clickonce-deployment.md)|省略可能です。  更新プログラムの配置とシステムへの公開に使用される属性を指定します。|`install`<br /><br /> `minimumRequiredVersion`<br /><br /> `mapFileExtensions`<br /><br /> `disallowUrlActivation`<br /><br /> `trustUrlParameters`|  
|[\<compatibleFrameworks\> 要素](../Topic/%3CcompatibleFrameworks%3E%20Element%20\(ClickOnce%20Deployment\).md)|必須。  このアプリケーションをインストールして実行できる .NET Framework のバージョンを指定します。|`SupportUrl`|  
...
|[\<signature\> 要素](../deployment/signature-element-clickonce-deployment.md)|省略可能です。  この配置マニフェストにデジタル署名するために必要な情報が含まれます。|なし|  
|[\<customErrorReporting\> 要素](../deployment/customerrorreporting-element-clickonce-deployment.md)|省略可能です。  エラー発生時に表示する URI を指定します。|URI|  
...
## 解説  
 ...
 詳細については、「[ClickOnce のセキュリティと配置](../deployment/clickonce-security-and-deployment.md)」を参照してください。  
