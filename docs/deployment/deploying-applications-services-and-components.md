...
ステップ バイ ステップ デプロイメントの手順のチュートリアルを参照してください。 Web アプリケーションの配置は、Visual Studio から最適な展開オプションを決定するより詳細な情報を必要がある場合を参照してください。[どのような発行オプションは私のですか?](../ide/not-in-toc/web-publish-options.md)です。
