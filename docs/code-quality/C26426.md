...
- ルールが constexpr 関数の呼び出しを無視するか、という前提に計算されるこれらのいずれかの組み込み関数がコンパイル時または実行時に予測可能な実行を保証します。
  ...
  - 静的クラス メンバーは、初期化子でもチェックされますので、グローバルと見なされます。
    ## <a name="example"></a>例
    外部のバージョンのチェック//api.cpp int api_version API_DEFAULT_VERSION; を =そのための非定数、実行時に変化を想定しています。
    int get_api_version() noexcept 戻り {api_version};

client.cpp int get_api_version() noexcept;bool is_legacy_mode get_api_version() を = < API_LEGACY_VERSION; を =C26426、古い値も
