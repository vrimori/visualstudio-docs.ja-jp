...
-   テスト対象のアプリケーションのモジュールごとに個別の <xref:Microsoft.VisualStudio.TestTools.UITest.Common.UIMap.UIMap> ファイルを作成します。  詳細については、「[複数の UI マップでの大規模アプリケーションのテスト](../test/testing-a-large-application-with-multiple-ui-maps.md)」を参照してください。  
...
## バック グラウンド プロセスが完了しないとテストを継続できない場合の作業  
