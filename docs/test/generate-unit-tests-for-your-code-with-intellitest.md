 ...
 **A:** はい。前提事項は、特定のメソッドの単体テストに、どのテスト データが必要ないかを指定するために使用します。 前提事項を追加するには、<xref:Microsoft.Pex.Framework.PexAssume> クラスを使用します。 たとえば、長さの変数が NULL ではないという前提事項は次のように追加します。  
