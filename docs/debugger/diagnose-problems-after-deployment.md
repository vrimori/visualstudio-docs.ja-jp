...
2.  イベント ページで、これらの呼び出しの実行時間を確認します。 実行ツリーで、時間がかかっている呼び出しを見つけます。
    ...
    - [他すべてのイベントと、IntelliTrace ログ内の情報とは](../debugger/using-saved-intellitrace-data.md)
    - [ここからすれば他に何をしますか。](#WhatElse)
    - [パフォーマンス イベントに関する詳細情報が必要ですか。](https://blogs.msdn.microsoft.com/devops/2013/09/20/performance-details-in-intellitrace/)
...
####  <a name="WhyInclude"></a> Q: 自分のプロジェクト、ソース管理、ビルド、およびリリースを使用したシンボルに関する情報を含めますか。
 ...
 TFS とこの情報を使用している場合は、ビルド マニフェスト (BuildInfo.config ファイル)、Visual Studio は、対応するソースと、現在接続されている TFS 上のシンボルではありません。 Visual Studio が適切な TFS または対応するソースを見つけられない場合、他の TFS を選択するように求めるプロンプトが表示されます。
