...
1. DLL と同じフォルダーまたは *.exe*ファイル。
   
1. シンボル ファイルのデバッガー オプションで指定された任意の場所。 追加すると、シンボルの場所を有効にする、次を参照してください。[シンボルの場所を構成し、オプションの読み込み](#BKMK_Specify_symbol_locations_and_loading_behavior)します。 
...
1.  (省略可能)シンボル読み込みのパフォーマンスを向上させるために**このディレクトリにシンボルをキャッシュ**、記号はシンボル サーバーをコピーできるローカル フォルダーのパスの種類。  
...
1. デバッガーからの読み込みを使用するモジュールの指定、**シンボル (.pdb) ファイルの場所**起動時。  
