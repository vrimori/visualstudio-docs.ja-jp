...
dev_langs: 
  ...
  - "C++"
helpviewer_keywords: 
  - "デバッグ [Visual Studio], セキュリティ"
  - "デバッガー、セキュリティ"
  - "セキュリティ [Visual Studio], デバッグのベスト プラクティス"
...
-   プロジェクトをインターネットからダウンロードするとき、およびそのプロジェクトを [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)] に読み込むときは注意します。 デバッグを実行しない場合でも、これはリスクの高い方法です。 この場合、含まれるプロジェクトとコードが信頼できるかどうかを判断します。  
 ...
 詳細については、「[マネージ コードのデバッグ](../debugger/debugging-managed-code.md)」を参照してください。  
...
### リモート デバッグのセキュリティ  
 ...
 ![&#91;エラー&#93; ダイアログ](../debugger/media/dbg_err_remotepermissionschanged.png "DBG\_ERR\_RemotePermissionsChanged")  
...
### Web サービスをデバッグするときのセキュリティ  
 ローカルでデバッグする方が安全ですが、[!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)] が Web サーバーにインストールされていない可能性があるため、ローカル デバッグを実行できない場合があります。 通常、Web サービスのデバッグは、開発時を除いてリモートで行われます。そのため、Web サービスのデバッグにも、リモート デバッグのセキュリティに関する推奨事項が適用されます。 次に、その他の推奨される手順を示します。 詳細については、「[Debugging XML Web Services](http://msdn.microsoft.com/ja-jp/c900b137-9fbd-4f59-91b5-9c2c6ce06f00)」を参照してください。  
