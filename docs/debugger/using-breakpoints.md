...
-   デバッグ中でないプロセスは、メモリ位置に書き込みます。  
-   メモリの場所は、2 つ以上のプロセスで共有されます。  
...
>  
>- 関数の終了時にローカル変数にデータ ブレークポイントを設定した場合、ブレークポイントを有効のままメモリ アドレスのブレークポイントの動作は予測可能なので、該当するではなく。 ローカル変数にデータ ブレークポイントを設定した場合は、削除または、関数が終了する前にブレークポイントを無効にする必要があります。  

##  <a name="BKMK_Specify_advanced_properties_of_a_breakpoint_"></a> [ブレークポイント] ウィンドウでブレークポイントを管理します。 
...
##  <a name="breakpoint-conditions"></a>ブレークポイント条件  
 条件を設定して、ブレークポイントを実行するタイミングと場所を制御することができます。 条件には、デバッガーによって認識される有効な式を指定できます。 有効な式の詳細については、次を参照してください。[デバッガーで式](../debugger/expressions-in-the-debugger.md)します。  
...
1. ドロップダウンで、次のように選択します。**条件式**、**ヒット カウント**、または**フィルター**、し、値を設定します。 
  
1. 選択**閉じます**またはキーを押します**Ctrl**+**」と入力**を閉じる、**ブレークポイントの設定**ウィンドウ。 またはから、**ブレークポイント**ウィンドウで、 **OK**ダイアログ ボックスを閉じます。 
...
選択すると**条件式**、2 つの条件から選択できます:**は**または**変更されたときに**します。 選択**は**式が満たされているときにブレークするまたは**変更されたときに**式の値が変更されたときに中断します。  
 ...
 >[!NOTE]
 >動作、**変更されたときに**フィールドは、さまざまなプログラミング言語によって異なります。 
...
3. を調査する時点で新しいブレークポイントを追加します。オブジェクトがコレクションに追加する場合など、表示します。 ブレークポイントを右クリックして**条件**します。  
   
4. オブジェクト ID を使用して、**条件式**フィールド。 たとえば場合、変数`item`、コレクションに追加するオブジェクトは、**が true**と種類**item $ = =\<n >** ここで、 \<n > オブジェクト ID 番号です.  
...
##  <a name="BKMK_Print_to_the_Output_window_with_tracepoints"></a> ブレークポイント アクションとトレース ポイント  
 A*トレース ポイント*メッセージを出力するブレークポイントです、**出力**ウィンドウ。 トレースポイントはプログラミング言語の一時的なトレース ステートメントのように機能できます。  
