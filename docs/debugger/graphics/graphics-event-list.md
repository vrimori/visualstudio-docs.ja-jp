...
### <a name="event-list-views"></a>イベント一覧ビュー  
 イベント一覧は、2 つの異なるビューをサポートしています。これらのビューは、ワークフローと環境設定をサポートするためにさまざまな方法でグラフィックス イベントを体系化しています。 最初のビューは、 *GPU 作業ビュー*イベントとそれらに関連付けられている状態を階層的に整理します。 2 つ目のビューは *タイムライン ビュー* で、これはイベントを時系列の単純なリストに体系化します。  
...
-   **グラフィックス イベント一覧**、イベントの一覧の上のウィンドウの検索、**ビュー**ドロップダウンいずれかを選択し、**タイムライン**ビューまたは**のGPUの処理**ビュー。  
