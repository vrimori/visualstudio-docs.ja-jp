...
デバッガーを効果的に使用するが、スキルは、時間と方法について説明しますが、すべてのソフトウェア開発者の基本的なタスクは、最終的にもです。 この記事でデバッグの中核となる原則を紹介し、開始するためのヒントを提供します。
...
* API を正しく使用していますか。 おそらく右 API を使用するが、適切な方法で使用していません。
...
1. をクリックして、**再起動**![アプリの再起動](../debugger/media/dbg-tour-restart.png "RestartApp")デバッグ ツールバーのボタン (**Ctrl** + **Shift**  +  **F5**)。
    ...
    アプリは、設定したブレークポイントで一時停止します。 黄色の強調表示には、デバッガーが一時停止していることを示します (コードの黄色い線がまだ実行)。
...
1. コードでクリックして`case 'l'`置き換えます`case 'I'`します。
