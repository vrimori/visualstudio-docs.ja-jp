...
ms.locfileid: "50050113"
---
# <a name="find-memory-leaks-with-the-crt-library"></a>CRT ライブラリを使用したメモリのリークを検出します。
...
1. **Enter** キーを押します。  
   ...
   デバッガーによって呼び出しが評価され、その結果が **[値]** 列に表示されます。 この値になります **-1**メモリ割り当てにブレークポイントを設定していない場合。  
...
オブジェクトにデータ ブレークポイントを設定する方法も役に立つことがあります。 詳細については、次を参照してください。[ブレークポイントを使用して](../debugger/using-breakpoints.md)します。  
