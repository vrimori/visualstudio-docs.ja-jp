...
> [!NOTE]
>  `CV_HREG_e` 列挙体は完全に複製するには大きすぎるため、省略されたバージョンが参照として提供されます。  欠落エントリは斜体で示されます。  完全な列挙体は、cvconst.h ヘッダー ファイルで使用できます。  
...
## Elements  
 CV\_REG\_\*  
 Intel 80x86とix86プロセッサで使用されるレジスタとPコード。  
  
 CV\_R68\_\*  
 68K プロセッサで使用されるレジスタ。  
  
 CV\_M4\_\*  
 MIPS 4000 シリーズ プロセッサで使用されるレジスタ。  
  
 CV\_ALPHA\_\*  
 デジタル イクィップメント社アルファAXPシリーズ プロセッサで使用されるレジスタ。  
  
 CV\_PPC\_\*  
 Motorola\/IBMのPowerPCプロセッサで使用されるレジスタ。  
  
 CV\_SH3\_\* および CV\_SH\_\*  
 日立SH3と日立SHシリーズ使用されるプロセッサのレジスタ。  
  
 CV\_ARM\_\*  
 Advanced RISC Machine \(ARM\) プロセッサで使用されるレジスタ。  
  
 CV\_IA64\_\*  
 IA64シリーズIntelプロセッサで使用されるレジスタ。  
  
 CV\_TRI\_\*  
 インフィニオンのテクノロジーズTriCoreのプロセッサで使用されるレジスタ。  
  
 CV\_AM33\_\*  
 Matsushita\/松下電器業界AM33と関連プロセッサで使用されたレジスタ。  
  
 CV\_M32R\_\*  
 三菱M32Rプロセッサで使用されるレジスタ。  
  
## 解説  
 プロセッサの種類は、それぞれ独自のレジスタ セットを使用します。  
