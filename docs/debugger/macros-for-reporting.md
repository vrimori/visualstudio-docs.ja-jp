...
特定のアプリケーションは、デバッグ レポート C ランタイム ライブラリに付属しているマクロを提供しない必要があることがあります。 このような場合は、独自の要件を満たすために特に設計マクロを記述できます。 たとえば、ヘッダー ファイルの 1 つに、次のような ALERT_IF2** というマクロを定義できます。  
