...
このトピックの最新バージョンをご覧[ResolveComReference タスク](https://docs.microsoft.com/visualstudio/msbuild/resolvecomreference-task)します。  
...
|`SdkToolsPath`|(省略可能) [String] (<!-- TODO: review code entity reference <xref:assetId:///String?qualifyHint=False&amp;autoUpgrade=True>  -->) パラメーター。<br /><br /> `ExecuteAsTool` が `true` の場合、このパラメーターを、対象となっているフレームワークのバージョンの SDK ツール パスに設定する必要があります。|  
