...
|CA1404|[CA1404: P/Invoke の直後に GetLastError を呼び出します](../code-quality/ca1404-call-getlasterror-immediately-after-p-invoke.md)|Marshal.GetLastWin32Error メソッドまたは同等に呼び出し[!INCLUDE[TLA2#tla_win32](../includes/tla2sharptla-win32-md.md)]GetLastError 関数が、直前の呼び出しでないオペレーティング システムにメソッドを呼び出します。|  
...
|CA1802|[CA1802: 適切な場所にリテラルを使用します](../code-quality/ca1802-use-literals-where-appropriate.md)|フィールドが static および read-only ([!INCLUDE[vbprvb](../includes/vbprvb-md.md)] では Shared および ReadOnly) として宣言され、コンパイル時に計算できる値によって初期化されています。 対象フィールドに割り当てられている値は、コンパイル時に計算であるため、宣言を const に変更 (で Const [!INCLUDE[vbprvb](../includes/vbprvb-md.md)]) の代わりに実行時のコンパイル時に値を計算するためのフィールドします。|  
