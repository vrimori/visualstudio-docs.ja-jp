...
ms.locfileid: "47546952"
---
# <a name="graphics-object-table"></a>グラフィックス オブジェクト テーブル
[!INCLUDE[vs2017banner](../includes/vs2017banner.md)]

このトピックの最新バージョンをご覧[グラフィックス オブジェクト テーブル](https://docs.microsoft.com/visualstudio/debugger/graphics/graphics-object-table)します。  
...
|**深さ**|3-D テクスチャ オブジェクトの深さ。 テクスチャが 3-D でない場合、値は 0 です。 他のオブジェクトの種類には適用されません。|  
...
### <a name="device-context-object"></a>デバイス コンテキスト オブジェクト  
 Direct3d11 および direct3d10 では、デバイス コンテキスト (**D3D11 Device Context**または**D3D10 Device**)、最も重要な状態情報を保持し、他のリンクには、オブジェクトは特に重要です現在設定されている状態オブジェクト。 デバイス コンテキストの詳細が新しいドキュメント ウィンドウに表示され、そこで情報の各カテゴリがそれぞれのタブに示されます。新しいイベントが選択されると、デバイス コンテキストが現在のデバイスの状態を反映するように変更されます。  
  
### <a name="buffer-object"></a>バッファー オブジェクト  
 バッファー オブジェクトの詳細 (D3D11 バッファーまたは D3D10 バッファー) が、新しいドキュメント ウィンドウに表示されます。ウィンドウには、バッファーの内容がテーブルで示され、バッファーの内容を表示する方法を変更するためのインターフェイスが提供されます。 **データをバッファーに**サポートのコピーをテーブルし、他のツールを使用できるように貼り付けます: Microsoft Excel など、その内容を調べる。 バッファーのコンテンツがの値に従って解釈されます、**形式**コンボ ボックスで、上部にある、**データ バッファー**テーブル。 このボックスには、次の表のデータ型で構成される複合データ形式で入力できます。 たとえば、"float int" には 32 ビット浮動小数点値の後に 32 ビット符号付き整数値が続く構造体がリストに表示されます。 指定した複合データ形式はコンボ ボックスに追加され、後で使用できます。  
  
 切り替えることも、**オフセットを表示**バッファー内の各要素のオフセットを表示または非表示にチェック ボックスをオンします。  
...
|**4 バイト**|32 ビットの符号付き整数値。 同じ**int**します。|  
|**8 バイト**|64 ビットの符号付き整数値。 同じ**int64**します。|  
|**xbyte**|8 ビットの 16 進値。|  
|**x2byte**|16 ビットの 16 進値。|  
|**x4byte**|32 ビットの 16 進値。 同じ**xint**します。|  
|**x8byte**|64 ビットの 16 進値。 同じ**xint64**します。|  
|**ubyte**|8 ビットの符号なし整数値。|  
|**u2byte**|16 ビットの符号なし整数値。|  
|**u4byte**|32 ビットの符号なし整数値。 同じ**uint**します。|  
|**u8byte**|64 ビットの符号なし整数値。 同じ**uint64**します。|  
|**半分**|16 ビット浮動小数点値。|  
|**half2**|2 個の 16 ビット浮動小数点値を含むベクター。|  
|**half3**|3 個の 16 ビット浮動小数点値を含むベクター。|  
|**half4**|4 個の 16 ビット浮動小数点値を含むベクター。|  
|**double**|64 ビット浮動小数点値。|  
|**int**|32 ビットの符号付き整数値。 同じ**4 バイト**します。|  
|**int64**|64 ビットの符号付き整数値。 同じ**8 バイト**します。|  
|**xint**|32 ビットの 16 進値。 同じ**x4byte**します。|  
|**xint64**|64 ビットの 16 進値。 同じ**x8byte**します。|  
|**uint**|32 ビットの符号なし整数値。 同じ**u4byte**します。|  
|**uint64**|64 ビットの符号なし整数値。 同じ**u8byte**します。|  
