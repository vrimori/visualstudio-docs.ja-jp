...
> [!NOTE]
> Visual Studio のプロジェクトに EditorConfig ファイルを追加するとき、ドキュメントの書式を設定しないかぎり、既存のコードの書式設定は変更されませ (**[編集]** > **[詳細設定]** > **[ドキュメントのフォーマット]**、または **Ctrl**+**K**、**Ctrl**+**D**)。 ただし、新しいコード行はすべて、EditorConfig の設定に従って書式設定されます。
