...
Visual Studio 2015 へようこそ。これは開発者用の生産性ツール、クラウド サービス、および拡張機能の統合されたスイートであり、お客様およびチームが、Web、Windows ストア、デスクトップ、Android および iOS 用のアプリやゲームを作成できるようにするものです。  
 ...
 Visual Studio ALM の多数の機能強化や新機能の詳細については、「[Visual Studio 2015 のアプリケーション ライフサイクル管理に関する新機能](http://msdn.microsoft.com/ja-jp/54b98a53-6083-4303-869a-8063d8fae938)」を参照してください。  
...
### ネイティブ C\+\+ 用のクロス プラットフォーム アプリおよびライブラリ  
 ...
 クロスプラットフォームの C\+\+ を使用する場合は、「[Visual C\+\+ を使ったクロスプラットフォーム アプリをビルドする](../misc/build-cross-platform-mobile-apps-with-visual-cpp.md)」を参照してください。  
...
#### .NET Framework  
 Microsoft [!INCLUDE[net_v46](../ide/includes/net_v46_md.md)] では、約 150 の API が新たに提供され、50 の API が更新されて、これまで以上に多くのシナリオに対応できるようになっています。 たとえば、さらに多くのコレクションで <xref:System.Collections.Generic.IReadOnlyCollection%601> が実装され、使いやすさの点で向上しています。 さらに前述の ASP.NET 5 では、最新のクラウド ベース アプリをビルドするための .NET プラットフォームがすっきりとした形で提供されています。  
  
 .NET Framework をターゲットとして C\# で書かれた Windows ストア アプリで、.NET Native を利用できるようになり、アプリを IL ではなくネイティブ コードにコンパイルすることが可能になりました。また、[!INCLUDE[net_v46](../ide/includes/net_v46_md.md)] には 64 ビット Just\-In\-Time \(JIT\) コンパイラーである RyuJIT も追加されています。  
 ...
 詳細については、「[新機能](../Topic/What's%20New%20in%20the%20.NET%20Framework.md)」および「[.NET Blog](http://blogs.msdn.com/b/dotnet/)」を参照してください。  
...
#### C\+\+  
 ...
 詳細については、「[Visual C\+\+ の新機能](/visual-cpp/top/what-s-new-for-visual-cpp-in-visual-studio-2015)」および「[Visual C\+\+ Team Blog](http://blogs.msdn.com/b/vcblog/)」を参照してください。  
...
-   **JavaScript \/ Cordova**。 JavaScript で Windows、iOS、Android のネイティブ アプリを作成するには、[Visual Studio Tools for Apache Cordova](http://msdn.microsoft.com/library/dn879821\(v=vs.140\).aspx) を使用します。  
     ...
     Cordova の Visual Studio デバッグ サポートについては、MSDN ライブラリの「[アプリのデバッグ](../Topic/Debug%20Your%20App%20Built%20with%20Visual%20Studio%20Tools%20for%20Apache%20Cordova.md)」で詳しく説明されています。  
...
-   **C\# \/ Xamarin**。[Xamarin](http://msdn.microsoft.com/library/dn879698\(v=vs.140\).aspx) を使用し、C\# を使って Visual Studio で Windows、iOS、および Android のネイティブ アプリを作成します。  
     ...
     デバッグ機能については、[Xamarin 開発者ガイド](http://developer.xamarin.com/guides)の「[Debugging](http://developer.xamarin.com/guides/ios/deployment,_testing,_and_metrics/debugging_in_xamarin_ios/)」 \(iOS\) および「[Debug on Device](http://developer.xamarin.com/guides/android/deployment,_testing,_and_metrics/debugging_with_xamarin_android/)」で説明されています。  
...
## デバッグと診断  
 デバッグの新機能については、「[Visual Studio 2015 のデバッガーの新機能](../debugger/what’s-new-for-the-debugger-in-visual-studio-2015.md)」を参照してください。  
  
 診断の新機能については、「[診断ツールの新機能](../profiling/what-s-new-in-profiling-tools.md)」を参照してください。  
...
### エラー一覧  
 エラー一覧はあらゆる列のフィルター処理をサポートするようになりました。 また入力時の C\# または Visual Basic ソリューション全体におけるエラー、警告、コード分析を、コードの変更によって大量の警告が発生したとしてもライブで表示します。 新しいエラー一覧は既存の使用方法と後方互換性があります。 詳細については、「[\[エラー一覧\] ウィンドウ](../Topic/Error%20List%20Window.md)」を参照してください。  
...
## ライブ コード分析 \(電球\)  
 ...
 ![Visual Studio コード エディターの電球](../ide/media/vs2015_lightbulbs.png "VS2015\_LightBulbs")  
  
## その他の IDE の強化された機能の活用  
...
### メニューの先頭文字の大文字指定  
 ...
 ![Visual Studio 2015 メニューの先頭文字の大文字指定コマンド](../ide/media/vs2015_mainmenu.png "VS2015\_MainMenu")  
...
### カスタム レイアウト  
 ...
 ![Visual Studio カスタム レイアウト メニュー項目](../ide/media/vs2015_customlayout.png "VS2015\_CustomLayout")  
...
### CodeLens: コードに何が起こったかを検索します \(Enterprise Edition と Professional Edition のみ\)。  
...
### 設計およびモデリング ツール \(Enterprise エディションのみ\)  
 **コード マップと依存関係グラフ**  
...
-   **クラス ビューおよびオブジェクト ブラウザーからダイアグラムを作成します**。 クラス ビューおよびオブジェクト ブラウザー ウィンドウから新規マップまたは既存マップにファイルとアセンブリをドラッグ アンド ドロップします。  
 ...
 「[ソリューション間の依存関係をマップする](../modeling/map-dependencies-across-your-solutions.md)」を参照してください。  
...
### 問題点、改善点の報告  
 ...
 タイトル バーにある黄色の笑顔マークをクリックしてから、**\[問題点、改善点の報告\]** をクリックします。 気になる点について記入した後、\[問題点、改善点の報告\] ボタンをクリックしてください。 詳細については、「[ご意見](../ide/talk-to-us.md)」を参照してください。  
  
### クラッシュ、ハング、パフォーマンスの問題についての報告  
...
## 参照  
 [Apache Cordova を使ったクロスプラットフォーム アプリをビルドする](../Topic/Build%20cross-platform%20apps%20with%20Visual%20Studio%20Tools%20for%20Apache%20Cordova.md)   
 ...
 [Visual C\+\+ を使ったクロスプラットフォーム アプリをビルドする](../misc/build-cross-platform-mobile-apps-with-visual-cpp.md)   
 ...
 [Visual Studio 2015 のアプリケーション ライフサイクル管理に関する新機能](http://msdn.microsoft.com/ja-jp/54b98a53-6083-4303-869a-8063d8fae938)