---
title: "メソッドの生成 - コード生成 (Visual Basic) | Microsoft Docs"
ms.custom: 
ms.date: 11/17/2016
ms.reviewer: 
ms.suite: 
...
1. 赤い波線がある行にカーソルを移動します。この波線は、使用したメソッドがまだ存在しないものであることを示しています。
   ...
   ![強調表示されたコード](media/method-highlight-vb.png)
...
1. 次に、以下のいずれかを実行します。
   ...
   * **マウス**
   ...
   ![メソッド生成のプレビュー](media/method-preview-vb.png)
...
1. メソッドと、その使用から推論されたすべてのパラメーターが、自動的に作成されます。
   ...
   ![メソッド生成の結果](media/method-result-vb.png)
