...
|[/DebugExe (devenv.exe)](../../ide/reference/debugexe-devenv-exe.md)|[!INCLUDE[vcprvc](../../code-quality/includes/vcprvc_md.md)] の実行可能ファイルをデバッガーの制御下に読み込みます。 このスイッチは、[!INCLUDE[vbprvb](../../code-quality/includes/vbprvb_md.md)] または [!INCLUDE[csprcs](../../data-tools/includes/csprcs_md.md)] の実行可能ファイルには使用できません。 詳細については、「[デバッガーでプロセスを自動的に開始する](../../debugger/debug-multiple-processes.md#BKMK_Automatically_start_an_process_in_the_debugger)」を参照してください。|  
