...
helpviewer_keywords: 
  - "コマンド [Visual Studio], ショートカット キー"
  - "カスタム ショートカット キー [Visual Studio]"
  - "カスタマイズ (キーボード ショートカットを) [Visual Studio]"
  - "エクスポート (ショートカット キーを) [Visual Studio]"
  - "インポート (ショートカット キーを) [Visual Studio]"
  - "キーボード ショートカット [Visual Studio], カスタマイズ"
  - "ショートカット キーの組み合わせ [Visual Studio]"
...
-   ショートカット キーを選択するときのコンテキスト。  たとえば、F2 ショートカット キーでは、設定デザイナーを使用している場合は Edit.EditCell コマンドが呼び出され、チーム エクスプローラーを使用している場合は File.Rename コマンドが呼び出されます。  
 ...
 設定、カスタマイズ、コンテキストにかかわらず、**\[オプション\]** ダイアログ ボックスでいつでもショートカット キーを変更できます。  多数のコマンドのショートカット キーについては、「[頻繁に使用するコマンドに対応する既定のキーボード ショートカット](../ide/default-keyboard-shortcuts-for-frequently-used-commands-in-visual-studio.md)」で確認できます。また、既定のショートカット キー \(\[全般的な開発設定\] に基づく\) の一覧については、「[既定のキーボード ショートカット](../ide/default-keyboard-shortcuts-in-visual-studio.md)」で確認できます。  
...
3.  **\[以下の文字列を含むコマンドを表示\]** ボックスに、コマンドの名前を空白を含めずに入力し始めます。  
...
4.  **\[キーボード\]** チェック ボックスをオンにして、**\[次へ\]** をクリックします。  
  
     ![カスタマイズされたキーボード ショートカットのみをエクスポート](../ide/media/exportshortcuts.png "ExportShortcuts")  
  
5.  **\[設定ファイルの名前を指定してください\]** と **\[設定を次のディレクトリに格納する\]** で、既定値のままにするか別の値を指定し、**\[完了\]** をクリックします。  
...
3.  **\[いいえ、現在の設定を上書きして新しい設定をインポートします\]** をクリックし、**\[次へ\]** をクリックします。  
  
4.  **\[ユーザーの設定\]** で、インポートするショートカット キーを含む設定ファイルを選択するか、**\[参照\]** をクリックして適切な設定ファイルを見つけます。  
  
5.  **\[次へ\]** ボタンをクリックします。  
  
6.  **\[インポートする設定を指定してください\]** で、**\[すべての設定\]** チェック ボックスをオフにし、**\[オプション\]**、**\[環境\]** の順に展開します。  
  
7.  **\[キーボード\]** チェック ボックスをオンにして、**\[完了\]** をクリックします。  
  
     ![カスタマイズされたキーボード ショートカットのみをインポート](../ide/media/importshortcuts.png "ImportShortcuts")  
