...
1. 言語名の隣にある三角形を選択し、使用するプログラミング言語のテンプレート一覧を表示し、プロジェクトの種類を選択します。

  次は、Visual C# Web プロジェクトで利用できるプロジェクト テンプレートの例です。
...
プロジェクトを作成するとき、そのプロジェクトで使用する .NET Framework のバージョンを指定できます。 .NET Framework バージョンを指定するには、**[新しいプロジェクト]** ダイアログ ボックスの **[.NET Framework]** バージョン ドロップダウン メニューを選択します。

![.NET Framework バージョン セレクター](./media/vside-newproject-framework.png)

> [!NOTE]
> .NET Core テンプレートからプロジェクトを作成する場合、ドロップダウンで選択した .NET Framework バージョンは無視されます。
...
1. 左側のウィンドウ (**[テンプレート]**) で、**[その他のプロジェクトの種類]** を選択し、展開された一覧で **[Visual Studio ソリューション]** を選択します。
