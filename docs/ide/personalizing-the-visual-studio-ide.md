...
カスタマイズ オプションは [[環境オプション] ダイアログ ボックス](../ide/reference/environment-options-dialog-box.md)でも公開されています。 さらに、設定の多くは Visual Studio のインスタンス間でローミングできます。 「[同期された設定](../ide/synchronized-settings-in-visual-studio.md)」を参照してください。
...
多くの環境設定をカスタマイズするには、**[クイック起動]** に「`environment`」と入力し、**Enter** キーを押します。 プロパティ ページが表示されたら、  **F1** キーを押すと、そのページのさまざまな設定に関するヘルプが表示されます。 詳細については、「[[環境] ([オプション] ダイアログ ボックス)](../ide/reference/environment-options-dialog-box.md)」を参照してください。  

## <a name="external-tools"></a>外部ツール
メニューおよびツールバーに表示される外部ツールをカスタマイズするには、「[Visual Studio の外部ツール](../ide/managing-external-tools.md)」を参照してください。  
