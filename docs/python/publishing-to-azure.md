...
Visual Studio には、Azure App Service に Python Web アプリを直接発行する機能が用意されています。 Azure App Service に発行することは、サーバーに必要なファイルをコピーし、アプリの起動方法を Web サーバーに指示する適切な `web.config` ファイルを設定することを意味します。 
