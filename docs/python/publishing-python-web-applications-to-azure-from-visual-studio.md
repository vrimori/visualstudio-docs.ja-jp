---
title: Azure App Service への Python アプリの発行 | Microsoft Docs
...
Visual Studio 2015 からデバッグ構成を発行すると、プロセスで `web.debug.config` ファイルが自動的に作成され、必要なデバッグ ツールを含む `ptvsd` フォルダーが追加されます。
