...
エラーは通常実行時まで表示されないため、プロジェクト全体の型ヒントを検証すると役立ちます。 このために、Visual Studio では、**ソリューション エクスプローラー**に **[Python]、[Mypy の実行]** のコンテキスト メニュー コマンドが統合されています。
