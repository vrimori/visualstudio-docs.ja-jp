---
title: "VSTO アドインのアーキテクチャ | Microsoft Docs"
ms.custom: ""
ms.date: "02/02/2017"
...
helpviewer_keywords: 
  - "VSTOLoader.dll"
  - "アーキテクチャ [Visual Studio での Office 開発]、アプリケーション レベルのアドイン"
  - "vstoee.dll"
  - "アプリケーション レベルのアドイン [Visual Studio での Office 開発]、アーキテクチャ"
  - "アドイン [Visual Studio での Office 開発]、アーキテクチャ"
...
-   [VSTO アドインと Microsoft Office アプリケーションの連携](#HowAddinsWork)  
 ...
 VSTO アドインの作成に関する一般情報については、「[Office ソリューションの開発の概要 &#40;VSTO&#41;](../vsto/office-solutions-development-overview-vsto.md)」および「[VSTO アドインのプログラミングについて](../vsto/getting-started-programming-vsto-add-ins.md)」を参照してください。  
...
##  <a name="UnderstandingAddIns"></a> VSTO アドインについて  
 ...
 アプリケーションに複数の VSTO アドインがインストールされている場合、それぞれの VSTO アドインは異なるアプリケーション ドメインに読み込まれます。 つまり、正しく動作しない 1 つの VSTO アドインが原因で他の VSTO アドインでエラーが発生することはありません。 また、アプリケーションが閉じられた場合に、すべての VSTO アドイン アセンブリを確実にメモリからアンロードするためにも役立ちます。 アプリケーション ドメインについて詳しくは、「[アプリケーション ドメイン](../Topic/Application%20Domains.md)」を参照してください。  
...
### レジストリ エントリ  
 ...
 ClickOnce を使用してソリューションを配置すると、発行プロセスで生成されたセットアップ プログラムによってエンド ユーザーのコンピューター上にレジストリ キーが自動的に作成されます。 詳細については、「[ClickOnce を使用した Office ソリューションの配置](../vsto/deploying-an-office-solution-by-using-clickonce.md)」を参照してください。  
  
### 配置マニフェストとアプリケーション マニフェスト  
 VSTO アドインは配置マニフェストとアプリケーション マニフェストを使用して、最新バージョンの VSTO アドイン アセンブリを特定して読み込みます。 配置マニフェストは、最新のアプリケーション マニフェストを指します。 アプリケーション マニフェストは、VSTO アドイン アセンブリを指し、アセンブリ内で実行するエントリ ポイント クラスを指定します。 詳細については、「[Office ソリューションにおけるアプリケーション マニフェストと配置マニフェスト](../vsto/application-and-deployment-manifests-in-office-solutions.md)」を参照してください。  
...
##  <a name="HowAddinsWork"></a> VSTO アドインと Microsoft Office アプリケーションの連携  
...
10. [!INCLUDE[vsto_runtime](../vsto/includes/vsto-runtime-md.md)] は、VSTO アドインの <xref:Microsoft.Office.Tools.AddInBase.RequestService%2A> メソッドを呼び出します \(このメソッドがオーバーライドされている場合\)。  
     ...
     必要に応じて、このメソッドをオーバーライドして、機能拡張インターフェイスを実装するオブジェクトを返すことで Microsoft Office 機能を公開できます。 詳細については、「[機能拡張インターフェイスによる UI 機能のカスタマイズ](../vsto/customizing-ui-features-by-using-extensibility-interfaces.md)」を参照してください。  
