---
title: "チュートリアル: サーバー上のブックへのデータの挿入 |Microsoft ドキュメント"
ms.custom: 
ms.date: 02/02/2017
ms.reviewer: 
ms.suite: 
...
---
# <a name="walkthrough-inserting-data-into-a-workbook-on-a-server"></a>チュートリアル : サーバー上のブックへのデータの挿入
  このチュートリアルを使用して Excel を起動しなくても Microsoft Office Excel ブックにキャッシュされているデータセットにデータを挿入する方法を示します、<xref:Microsoft.VisualStudio.Tools.Applications.ServerDocument>クラスです。  
...
-   作成する、<xref:Microsoft.Office.Tools.Excel.ListObject>ブック内のデータセットにバインドされています。  
  
-   データ キャッシュに、ブック内のデータセットを追加します。  
  
-   Excel を起動せず、コンソール アプリケーションでコードを実行して、キャッシュされたデータセットにデータを挿入します。  
...
5.  この接続を選択しをクリックして、AdventureWorksLT データベースへの既存の接続があれば、**次**です。  
     ...
     それ以外の場合は、 **[新しい接続]**をクリックし、 **[接続の追加]** ダイアログ ボックスを使用して新しい接続を作成します。 詳細については、次を参照してください。[する方法: データベース内のデータへの接続](../vsto/walkthrough-inserting-data-into-a-workbook-on-a-server.md)です。  
...
## <a name="checkpoint"></a>チェックポイント  
 ビルドして、Excel ブック プロジェクトをコンパイルおよび実行エラーが発生しないことを確認を実行します。  
...
1.  **ソリューション エクスプ ローラー**を右クリックし、 **AdventureWorksReport**プロジェクト**デバッグ**、順にクリック**新しいインスタンスを開始**です。  
     ...
     プロジェクトがビルドされ、Excel でブックを開きます。 <xref:Microsoft.Office.Tools.Excel.ListObject>で**Sheet1**が空で、ため、`adventureWorksLTDataSet`データ キャッシュ内のオブジェクトはまだありませんデータ。 次のセクションで、設定に、コンソール アプリケーションを使用するが、`adventureWorksLTDataSet`データを持つオブジェクト。  
...
## <a name="adding-data-to-the-cached-dataset-by-using-the-console-application"></a>コンソール アプリケーションを使用してデータをキャッシュされたデータセットを追加します。  
 使用して、<xref:Microsoft.VisualStudio.Tools.Applications.ServerDocument>データをブックにキャッシュされたデータセットを作成するコンソール アプリケーション内のクラスです。  
...
2.  **.NET** ] タブで [ **Microsoft.VisualStudio.Tools.Applications.ServerDocument**です。  
...
8.  `Main` メソッドに次のコードを追加します。 このコードは、次のオブジェクトを宣言します。  
    ...
    -   インスタンス、`AdventureWorksLTDataSet`と`ProductTableAdapter`で定義されている型、 **AdventureWorksDataSet**プロジェクト。  
...
9. 次のコードを追加、`Main`メソッド、前の手順で追加したコードの後にします。 このコードは次のタスクを実行します。  
    ...
    -   テーブル アダプターを使用して、型指定されたデータセット オブジェクトを塗りつぶします。  
    ...
    -   使用して、<xref:Microsoft.VisualStudio.Tools.Applications.CachedDataItem.SerializeDataInstance%2A>ローカル型指定されたデータセットからのデータでキャッシュされたデータセットを作成するメソッド。  
...
10. **ソリューション エクスプ ローラー**を右クリックし、 **DataWriter**プロジェクトをポイントし、**デバッグ**、クリックして**新しいインスタンスを開始**です。  
     ...
     プロジェクトがビルドされ、コンソール アプリケーションがローカルのデータセットと、アプリケーションは、ブック内のキャッシュされたデータセットにデータを保存するときに、いくつかのステータス メッセージを表示します。 アプリケーションを終了するには、enter キーを押します。  
  
## <a name="testing-the-workbook"></a>ブックのテスト  
 ブックを開くと、<xref:Microsoft.Office.Tools.Excel.ListObject>コンソール アプリケーションを使用して、キャッシュされたデータセットに追加されたデータが表示されます。  
...
3.  いることを確認、<xref:Microsoft.Office.Tools.Excel.ListObject>ブックを開いた後にデータが格納されます。  
