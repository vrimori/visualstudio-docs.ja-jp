...
-   データ ソースへのデータ バインディング。  詳細については、「[コンテンツ コントロールへのデータのバインディング](#DataBinding)」を参照してください。  
 ...
 ![ビデオへのリンク](~/data-tools/media/playvideo.gif "ビデオへのリンク") 関連のビデオ デモについては、「[Visual Studio Tools for the Office System \(3.0\) を使用した Word 2007 コンテンツ コントロールへのデータのバインディング](http://go.microsoft.com/fwlink/?LinkId=136785)」を参照してください。  
