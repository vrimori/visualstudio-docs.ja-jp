...
Linux 用のリモート R サービスは現在、rtvs デーモンとしてパッケージ化されています。 デーモンは、Ubuntu 16.04/16.10 LTS のデスクトップ、サーバー、および Ubuntu を実行している Windows Subsystem for Linux でサポートされテストされています。 このトピックの大部分は、このような各種システム上でリモート R サービスを設定するための手順説明となります。
