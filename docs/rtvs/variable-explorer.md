...
![Visual Studio の変数エクスプローラー ウィンドウ](~/docs/rtvs/media/variable-explorer-window.png)
...
![変数エクスプローラー](~/docs/rtvs/media/variable-explorer-table-view.png)
...
![2 つの列を基準にしたデータの並べ替えを示すテーブル ビュー。](~/docs/rtvs/media/variable-explorer-table-view-sorting.png)
...
さらに処理や分析を行うには、多くの場合、セッションの変数を CSV にエクスポートすると便利です。エクスポートは、変数エクスプローラーの各ノードの横にある小さい Excel のアイコン (![Excel エクスポート アイコン](~/docs/rtvs/media/variable-explorer-excel-icon.png)) を使用するか、項目を右クリックして **[CSV アプリで開く]**を選択します。 このアイコンを選択すると、`%userprofile%\Documents\RTVS_CSV_Exports` フォルダーの新規 CSV ファイルにデータが書き込まれ、そのファイルが起動されて、`.csv` の拡張子に関連付けられているアプリケーションで開きます。

## <a name="scopes"></a>スコープ

既定では、変数エクスプローラーはグローバル スコープで開かれます。 ウィンドウ上部のドロップダウンからパッケージを選択することで、パッケージ スコープに切り替えることができます。

![パッケージ スコープが表示された変数エクスプローラー](~/docs/rtvs/media/variable-explorer-package-scopes.png)

また、デバッガーのブレークポイントで停止したとき、関数スコープに切り替えることもできます (変数エクスプローラーがデバッグ中のコードの関数スコープに自動的に切り替わることはありません)。

![デバッグ中のデータ フレームが表示された変数エクスプローラー](~/docs/rtvs/media/variable-explorer-as-locals-window.png)
...
![[データセットのインポート] ダイアログ ボックス](~/docs/rtvs/media/variable-explorer-import-dataset-dialog.png)
