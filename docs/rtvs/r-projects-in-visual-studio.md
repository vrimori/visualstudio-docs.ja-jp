...
`.Rhistory` では、[[R インタラクティブ]](interactive-repl-for-r-in-visual-studio.md) ウィンドウに入力したすべてのコマンドが記録されます。 **[R ツール] > [Windows] > [履歴]** コマンドで、専用の履歴ウィンドウを開くことができます。 そのウィンドウには、履歴の内容を消去するためのツール バー ボタンとコンテキスト メニュー項目が含まれています。
...
プロジェクトのプロパティ ページを開くには、**ソリューション エクスプローラー**でプロジェクトを右クリックし、**[プロパティ]** を選択するか、**[プロジェクト] > [<プロジェクト名> のプロパティ]* メニュー項目を選択します。 開いているウィンドウには、次のようなプロジェクトのプロパティが表示されます。
...
R Tools for Visual Studio (RTVS) の右クリック メニューには、R プロジェクト、およびプロジェクト内のファイルとフォルダー用の独自のコマンドが多数追加されています。
...
| [ストアド プロシージャの公開] | プロセスを開始して、R スクリプトにストアド プロシージャが含まれている場合はそれを公開します。 「[SQL Server ストアド プロシージャの使用](integrating-sql-server-with-r.md#working-with-sql-server-stored-procedures)」を参照してください。 | 
...
| SQL クエリ | 空の `.sql` ファイル。 「[SQL Server Integration](integrating-sql-server-with-r.md)」(SQL Server の統合) を参照してください。 |
| R を使用したストアド プロシージャ | 子の SQL クエリと子のストアド プロシージャ テンプレート ファイルを含む R ファイル。 「[SQL Server Integration](integrating-sql-server-with-r.md)」(SQL Server の統合) を参照してください。 |
