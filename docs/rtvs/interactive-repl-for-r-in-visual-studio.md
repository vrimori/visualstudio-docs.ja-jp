...
**[リセット]** ボタンまたは **[R Tools]、[セッション]、[リセット]** を使用すると、セッション コンテキストが消去されます。 リモート セッションを使用している場合、リセットによりリモート コンピューター上のユーザー プロファイルも削除され、そこに格納されているすべてのファイルが消去されます  (「[ワークスペース](r-workspaces-in-visual-studio.md#directories-on-local-and-remote-computers)」を参照)。
