...
- lint オプション ([lint](linting-r-code.md) に関するページを参照): **[ツール]、[オプション]** メニューの順に移動し、**[テキスト エディター]、[R]、[Lint]** の順に選択します。
...
これらのオプションにより、[変数エクスプローラー](variable-explorer.md)と、ウォッチやローカルのような、デバッガー ウィンドウにおける値の処理方法が制御されます (「[デバッグ](debugging-r-in-visual-studio.md)」を参照してください)。
...
| ワークスペースを切り替える前に確認ダイアログを表示する | `Yes` | `No` に設定すると、ワークスペースを切り替えるとき、ユーザーに確認が求められません。 「[ワークスペースの切り替え](r-workspaces-in-visual-studio.md#switching-between-workspaces)」をご覧ください。 |
