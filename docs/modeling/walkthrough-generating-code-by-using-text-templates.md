...
厳密に型指定のバージョンでは、XML スキーマの変更は、クラスに変更の結果します。 コンパイラには、変更する必要があるアプリケーション コードの部分が強調表示されます。 汎用的な XML コードを使用する型指定されていないバージョンの場合、このようなサポートはありません。
...
テキスト テンプレート ファイルを追加し、セットの出力の拡張機能 *.cs*します。
...
## <a name="develop-the-text-template"></a>テキスト テンプレートを開発します。
...
## <a name="complete-the-text-template"></a>テキスト テンプレートを完了します。
...
### <a name="run-the-test-program"></a>テスト プログラムを実行します。
...
### <a name="write-and-update-the-application"></a>記述し、アプリケーションを更新
...
クラスを再生成、サンプルの XML ファイルが変更されたときに、次のようにクリックします。**すべてのテンプレートの変換**で、**ソリューション エクスプ ローラー**ツールバー。
...
## <a name="troubleshoot-the-text-template"></a>テキスト テンプレートをトラブルシューティングします。
