...
|デザイン時テンプレートの処理時または実行時 (前処理された) テンプレートのコンパイル時に "式が複雑すぎます" と表示されます。<br /><br /> [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)] 実行時テンプレートによって生成されたコードの検査を試みたときに動作しなくなります。|テキスト ブロックが長すぎます。 T4 は、テンプレートの行ごとに 1 つのリテラル文字列を使用して、テキスト ブロックを文字列連結式に変換します。 非常に長いテキスト ブロックには、コンパイラのサイズの制限を起こしたりことができます。|次のような式ブロックで長いテキスト ブロックを分割します。<br /><br /> `<#= "" #>`|  
