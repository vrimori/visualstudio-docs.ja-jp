...
1.  [Commands.vsct 内のコマンドを宣言します。](#VSCT)

2.  [Package.tt でパッケージのバージョン番号を更新](#version)です。 Commands.vsct を変更するときには必ずこの操作を実行してください。

3.  [CommandSet クラスにメソッドを記述](#CommandSet)を行うには、コマンドを実行する内容を定義して、コマンドを表示します。

 サンプルについては、次を参照してください。、 [Visualization and Modeling SDK の web サイト](http://go.microsoft.com/fwlink/?LinkID=185579)です。

> [!NOTE]
>  [切り取り]、[貼り付け]、[すべて選択]、[印刷] など、既存の一部のコマンドの動作を変更することもできます。このためには、CommandSet.cs でメソッドをオーバーライドします。 詳細については、次を参照してください。[する方法: 標準メニュー コマンドを変更](../modeling/how-to-modify-a-standard-menu-command-in-a-domain-specific-language.md)です。
...
1.  **ソリューション エクスプ ローラー**下で、 **DslPackage**プロジェクト、Commands.vsct を開きます。

2.  `Commands` 要素に 1 つ以上のボタンと 1 つのグループを定義します。 A*ボタン*メニュー項目。 A*グループ*メニュー内のセクションは、します。 これらの項目を定義するため、次の要素を追加します。
    ...
    > [!NOTE]
    >  各ボタンとグループは、GUID と整数の ID によって識別されます。 同じ GUID を使用して複数のグループとボタンを作成できます。 ただし、それぞれに異なる ID が必要です。 GUID の名前と ID の名前は、実際の Guid と内の数値 Id に変換されます、`<Symbols>`ノード。

3.  ドメイン固有言語のコンテキストでのみコマンドが読み込まれるようにするため、コマンドに表示制限を追加します。 詳細については、次を参照してください。 [VisibilityConstraints 要素](../extensibility/visibilityconstraints-element.md)です。
...
5.  `{000...000}` を、グループとメニュー項目を識別する GUID に置き換えます。 新しい GUID を取得するを使用して、 **GUID の作成**ツールを**ツール**メニュー。
...
### <a name="define-what-the-command-does"></a>コマンドが実行する処理を定義する
 ...
 モデル要素を変更する場合は、トランザクション内部で変更を行う必要があります。 詳細については、次を参照してください。[する方法: 標準メニュー コマンドを変更](../modeling/how-to-modify-a-standard-menu-command-in-a-domain-specific-language.md)です。
...
```
 ...
 モデルでは、オブジェクトからオブジェクトを移動する方法およびオブジェクトとリンクを作成する方法についての詳細については、次を参照してください。[する方法: 標準メニュー コマンドを変更](../modeling/how-to-modify-a-standard-menu-command-in-a-domain-specific-language.md)です。
...
```
 ...
 挿入されると、同じ GUID 値を使用して**Commands.vsct**です。
...
2.  キーを押して**f5 キーを押して**をソリューションをリビルドし、実験用のビルドにおけるドメイン固有言語のデバッグを開始します。
...
-   DSL パッケージをインストールするまでは、コマンドは Visual Studio のデバッグ インスタンスでのみ表示されます。 詳細については、次を参照してください。[ドメイン固有言語ソリューションの配置](../modeling/deploying-domain-specific-language-solutions.md)です。
 ...
 **誤ったメニュー テキストが表示されたら、または間違った場所でのコマンドが表示されます**:
