...
一般に、使用する、*ドメイン固有言語デザイナー* DslDefinition.dsl ファイルを編集します。 ただし DslDefinition.dsl ファイルの未加工の形式は XML であるため、DslDefinition.dsl ファイルを XML エディターで開くことができます。 ファイルに記述されている情報と、ファイルの編成を理解しておくと、デバッグおよび拡張を行う際に役立つことがあります。

このトピックに記載されている例は、コンポーネント図のソリューション テンプレートのものです。 例を確認するには、コンポーネント モデルのソリューション テンプレートに基づくドメイン固有言語ソリューションを作成します。 ソリューションの作成後に、DslDefinition.dsl ファイルが Domain-Specific Language Designer に表示されます。 ファイルを閉じるには、右クリックして **ソリューション エクスプ ローラー**、 をポイント**ファイルを開く**、 をクリックして**XML エディター**、クリックして**ok**です。

## <a name="sections-of-the-dsldefinitiondsl-file"></a>DslDefinition.dsl ファイルの各セクション

ルート要素は、 \<Dsl >、メジャーおよびマイナー バージョン番号のバージョン管理とその属性が、ドメイン固有言語、名前空間の名前を特定します。 `DslDefinitionModel` スキーマは、有効な DslDefinition.dsl ファイルの内容と構造を定義します。

子要素、 \<Dsl > ルート要素を次に示します。
...
このセクションを定義する方法、 **DSL のエクスプ ローラー**ユーザーが、モデルを編集するときにウィンドウが表示されます。
...
このセクションでは、連動するデザイナー (エディター) を定義、**ツールボックス**検証の設定、ダイアグラム、およびシリアル化方式です。 Designer セクションでは、モデルのルート クラスも定義されます。これは通常、図のルート クラスでもあります。

### <a name="explorer"></a>エクスプローラー

このセクションで、 **DSL のエクスプ ローラー** (XmlSerializationBehavior セクションで定義されている) の動作です。
...
-   **属性**です。 この属性には、生成されたクラスの CLR 属性が含まれています。

-   **BaseClass**です。 基底クラスを指定する場合、同じ型でなければなりません。 たとえば、ドメイン クラスにはその基底クラスとして別のドメイン クラスを指定する必要があり、コンパートメント シェイプにはコンパートメント シェイプを含める必要があります。 基底クラスを指定しない場合は、生成されるコード内のクラスは標準フレームワーク クラスから派生されます。 たとえば、ドメイン クラスは `ModelElement` から派生します。

-   **プロパティ**です。 この属性には、トランザクション制御で保持され、モデルを保存するときに維持されるプロパティが含まれています。

-   **ElementMergeDirectives**です。 各要素マージ ディレクティブは、別のクラスの異なるインスタンスがどのように親クラス インスタンスに追加されるかを制御します。 このトピックの後半で、要素マージ ディレクティブの詳細について説明します。
...
-   **IsElementName**です。 この属性を true に設定すると、親クラスのインスタンスの作成時にその値が自動的に固有値に設定されます。 各クラスで 1 つの文字列型プロパティのみに対して、この属性を true に設定できます。 コンポーネント図の例では、`Name` の `NamedElement` プロパティで `IsElementName` が true に設定されています。 ユーザーが `Component` 要素 (`NamedElement` から継承) を作成するときは常に、名前が自動的に初期化され、"Component6" のようになります。

-   `DefaultValue`。 この属性を指定した場合、このクラスの新規インスタンスでは、指定した値がこの属性に割り当てられます。 `IsElementName` が設定されている場合、DefaultValue 属性は新しい文字列の初期部分を指定します。

-   **カテゴリ**、ヘッダーをプロパティが表示されますが、**プロパティ**ウィンドウです。
...
-   **IsEmbedding**です。 このブール属性は、リレーションシップが埋め込みツリーの一部であるかどうかを指定します。 すべてのモデルは、その埋め込みリレーションシップを含むツリーを形成する必要があります。 したがって、モデルのルート以外のすべてのドメイン クラスは、1 つ以上の埋め込みリレーションシップのターゲットである必要があります。

-   **AllowsDuplicates**です。 このブール属性は、ソースとターゲットの両方で多重度が "many" であるリレーションシップにのみ適用されます。既定では false に設定されています。 言語ユーザーが、同一リレーションシップの複数のリンクを使用して、ソース要素とターゲット要素の 1 つのペアを接続できるかどうかを指定します。

## <a name="designer-and-toolbox-tabs"></a>[デザイナー] タブと [ツールボックス] タブ

メイン部分、**デザイナー** DslDefinition.dsl ファイルのセクションは、 **ToolboxTab**要素。 2 つのデザイナーは、生成されたデザイナーでの方向のセクションを表します。 それぞれが、これらの要素のいくつかを持つことができます**ツールボックス**です。 各**ToolboxTab**要素は、1 つまたは複数を含めることができます**ElementTool**要素、 **ConnectionTool**要素、またはその両方です。
...
このモデルに対して (C#) を作成するときに関連するクラスの各関係を生成するプロパティを使用して 1 つの手順のリンクを経由ジャンプできます。
...
言語のユーザーがから項目をドラッグすると、**ツールボックス**を図に、ツールのクラスのインスタンスを構築します。 また、そのインスタンスと既存のモデル要素の間にリンクが作成されます。 コンポーネントやコメントなど、いくつかの項目が作成されますから言語のユーザーがドラッグしたときに、**ツールボックス**図の空白部分にします。 その他の項目は、言語ユーザーがそれらを他のホスト要素にドラッグすると作成されます。 たとえば OutPort または Inport は、言語ユーザーがコンポーネントにドラッグすると作成されます。
...
-   **XmlRelationshipData**関係ごとに、クラスに属しています。 (リレーションシップにはそれ自体の XmlClassData ノードも含まれています。)
...
-   **MonikerElementName**ユーザーのシリアル化されたモデル ファイルにします。 この属性により、このクラスを参照するモニカーが導入されます。

-   **MonikerAttributeName**モニカーの XML 属性の名前を示すです。 ドメイン固有言語の作成者によって定義されているユーザーのシリアル化されたファイルのこのフラグメント内の**MonikerElementName** "inPortMoniker"としてと**MonikerAttributeName** "path"として。
...
**XmlName** XML で表示される属性は、対応する属性名を指定します。 規則では、この文字列は、最初の文字が小文字であることを除きプロパティ名と同一です。

既定では、**表現**属性が属性に設定します。 場合**表現**子要素に設定されて、XML にノードを作成します。 場合**表現**は無視に設定すると、このプロパティはシリアル化されません。

**IsMonikerKey**と**IsMonikerQualifier**属性は親クラスのインスタンスを識別するときに、ロール プロパティを提供します。 設定することができます**IsMonikerKey**で定義されているまたはクラスによって継承されている 1 つのプロパティを true に設定します。 この属性は、親クラスの個別のインスタンスを識別します。 `IsMonikerKey` に設定するプロパティは通常、名前またはその他のキー識別子です。 たとえば `Name` 文字列プロパティは、NameElement およびその派生クラスのモニカー キーです。 ユーザーがモデルをファイルに保存する場合、この属性には、埋め込みリレーションシップ ツリーの兄弟間で固有な各インスタンスの値が含まれている必要があります。
...
設定することができます、 **IsMonikerQualifier**文字列プロパティの属性し、要素の完全な名前を構築するために 1 つの手段を提供します。 たとえば、DslDefinition.dsl ファイルで**Namespace**モニカー修飾子は、します。
...
**XmlRelationshipData**属性、 **XmlClassData**属性は、ソース要素内で子ノードが入れ子になって方法だけを定義します。 ドメイン クラスのソースであるすべてのリレーションシップが 1 つ**XmlRelationshipData**属性。

**DomainRelationshipMoniker**属性は、クラスのソースとのリレーションシップのいずれかを識別します。

**RoleElementName**属性は、シリアル化データで子ノードを囲む XML タグ名を指定します。
...
場合、 **UseFullForm**属性に設定されている場合は true、入れ子構造の強化が導入されました。 このレイヤーはリレーションシップ自体を表します。 リレーションシップにプロパティが含まれている場合、この属性を true に設定する必要があります。
...
-   **Dsl** RootClass ノードであり、図のクラスです。 DomainClass、DomainRelationship、およびその他の要素は `Dsl` の下に埋め込まれています。

-   **クラス**は、 **RoleElementName**ドメイン固有言語および DomainClass 間の関係。
...
-   **XmlSerializationBehavior**下にある属性が埋め込まれている、`Dsl`属性、ですが、 **OmitElement**埋め込みリレーションシップに属性が設定されています。 したがって、その間に `RoleElementName` 属性がありません。 これに対し、**クラス データ**属性は、`RoleElementName`埋め込みのリレーションシップの属性、 **XmlSerializationBehavior**属性および**XmlClassData**属性。
...
-   **ExposesFillColorAsProperty**といくつかのような属性です。 これらのブール属性により、対応するプロパティをユーザーが変更できるようになります。 一般に、言語のユーザーは、ダイアグラムの図形をクリックすると、プロパティに表示される、**プロパティ**ウィンドウは、図形がマップされているドメイン クラスのインスタンスのものです。 `ExposesFillColorAsProperty` が true に設定されている場合、シェイプ自体のプロパティも表示されます。

-   **ShapeHasDecorators**です。 この属性のインスタンスは、テキスト、アイコン、および展開/折りたたみデコレータごとに生成されます。 (DslDefinition.dsl ファイルでは、`ShapeHasDecorators` は `UseFullForm` が true に設定されたリレーションシップです。)
