...
ms.locfileid: "39511812"
---
# <a name="customize-file-storage-and-xml-serialization"></a>ファイル格納処理および XML シリアル化処理をカスタマイズする

ユーザーは、インスタンスを保存するときにまたは*モデル*でドメイン固有言語 (DSL) の[!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)]、XML ファイルを作成または更新します。 ファイルは、ストア内のモデルを再作成を再読み込みすることができます。
