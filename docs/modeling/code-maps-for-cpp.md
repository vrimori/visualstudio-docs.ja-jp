---
title: C++ ソース ファイルとヘッダー ファイルの間の依存関係を表示します。
ms.date: 05/16/2018
ms.topic: conceptual
author: gewarren
ms.author: gewarren
...
Visual C++ のプロジェクトを含むソリューションを開くときは、IntelliSense データベースの更新に時間がかかる場合があります。 この期間中にするできないことがありますヘッダーのコード マップを作成する (*.h*または`#include`) ファイルを IntelliSense データベースの更新が終了するまでです。 更新の進行状況は、Visual Studio のステータス バーで監視できます。

- すべてのソース ファイルとヘッダー ファイル、ソリューション間の依存関係を表示するには、次のように選択します。**アーキテクチャ** > **インクルード ファイルのグラフの生成**です。
...
これらの問題は、C および C++ コードのコード マップを作成するときに発生する可能性があります。
...
|IntelliSense 参照データベースの特定の設定が無効になる。|Visual Studio の IntelliSense の特定の設定を無効にすることがあります**オプション** ダイアログ ボックス。|設定をオンにして有効にします。<br /><br /> 参照してください[オプション、テキスト エディター、C/C++、高度な](../ide/reference/options-text-editor-c-cpp-advanced.md)します。|
