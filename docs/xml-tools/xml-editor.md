...
XML エディターは [!INCLUDE[msCoName](../xml-tools/includes/msconame_md.md)] Visual Studio のテキスト エディターに基づいており、XML 言語のサポートが追加されています。 XML エディターには次の機能が含まれます。  
...
XML エディターは、よく知られている .xml、.xsd、.xsl、.config などのファイル拡張子に対して呼び出されます。ファイルに XML が含まれている場合は、未知のファイル拡張子についても XML エディターが呼び出されます。 開くことも任意のファイル、XML エディターでを使用して、**ファイルを開く**オプションと XML エディターを一覧から選択します。  
