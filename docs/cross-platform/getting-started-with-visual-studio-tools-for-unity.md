...
#### 2 \- Visual Studio Tools for Unity を使用するための Unity エディターの構成  
 ...
 Visual Studio Tools for Unity の組み込みサポートが有効であることを確認するため、**\[About Unity\]** ダイアログを表示します。 Unity エディターのメイン メニューで、**\[Help\]、\[About Unity\]** を選びます。Visual Studio Tools for Unity がインストールされており、正しく構成されている場合は、**\[About Unity\]** ダイアログの左下隅にメッセージが表示されます。  
  
 最後に、**\[Build Settings\]** ページでビルド ターゲットが設定されていることと、**\[Script Debugging\]** が有効になっていることをご確認ください。  
  
 ![デバッグのための Unity のビルド設定を構成します。](../cross-platform/media/vstu_debugging_build_settings.png "vstu\_debugging\_build\_settings")  
...
#### 2 \- Visual Studio Tools for Unity を使用するための Unity エディターの構成  
 ...
 ![Unity プロジェクトに VSTU パッケージをインポートします。](../cross-platform/media/vstu_configure_unity_import_vstu.png "vstu\_configure\_unity\_import\_vstu")  
...
#### 3 \- Unity エディターからの Visual Studio の起動  
 ...
 ![Visual Studio で Unity プロジェクトを開きます。](../cross-platform/media/vstu_configure_open_in_visual_studio.png "vstu\_configure\_open\_in\_visual\_studio")  
...
## 参照  
 [Unity のホーム ページ](http://unity3d.com)