...
4.  試してみましょう。キーを押して**F5**実験用インスタンスを起動します。 実験用インスタンスでの c# ユニバーサル ハブ アプリ プロジェクトの作成 (で、**新しいプロジェクト**ダイアログ ボックスで、 **Visual c#** > **Windows**  >  **Windows 8** > **ユニバーサル** > **ハブ アプリ**) に移動して、**ツール**メニューをクリックします**InvokeTestUniversalProject**、およびチェックインし、テキスト、**出力**ウィンドウ。 次のように表示されます。  
