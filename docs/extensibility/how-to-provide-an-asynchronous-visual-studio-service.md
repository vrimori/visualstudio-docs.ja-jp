...
1.  VSIX プロジェクトを作成 (**ファイル/新しい/プロジェクト/Visual c#/Extensiblity/VSIX プロジェクト**)。 プロジェクトに名前を**TestAsync**です。  
  
2.  VSPackage をプロジェクトに追加します。 プロジェクト ノードを選択、**ソリューション エクスプ ローラー** ] をクリック**追加/[新しい項目/Visual c# アイテム/機能拡張/Visual Studio パッケージ**です。 このファイルの名前を付けます**TestAsyncPackage.cs**です。  
