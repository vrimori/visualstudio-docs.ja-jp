...
-   [エディターと言語サービスの拡張機能](../extensibility/editor-and-language-service-extensions.md): IntelliSense が提供する Visual Studio の言語では、独自のカスタマイズを追加するか新しいプログラミング言語のサポートを作成します。 新しいステートメント入力候補、提案、および新しいクイック ツールヒントが表示を作成することができます。 電球アイコンから、新しいプログラミング言語をサポートするためにリファクタリングの候補を追加したり、コードの修正を追加することができます。
