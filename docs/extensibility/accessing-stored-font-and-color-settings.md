...
-   使用して、<xref:Microsoft.VisualStudio.Shell.Interop.IVsFontAndColorStorage.OpenCategory%2A>を開くには、カテゴリ、カテゴリの GUID とモード フラグを引数としてを使用して永続化するメソッド。  
    ...
    -   すべての設定またはだけユーザーを変更するを使用して取得できる、<xref:Microsoft.VisualStudio.Shell.Interop.IVsFontAndColorStorage>インターフェイスです。  
...
-   設定して、レジストリに格納されている設定を取得します。  
 ...
 既定では、イベントの生成は無効です。 イベントの生成を有効にするを使用してカテゴリを開く<xref:Microsoft.VisualStudio.Shell.Interop.__FCSTORAGEFLAGS>です。 これが原因で、適切な呼び出しに IDE <xref:Microsoft.VisualStudio.Shell.Interop.IVsFontAndColorEvents> VSPackage を実装するメソッド。  
