---
title: "値の変更の実装のサンプル | Microsoft Docs"
ms.custom: ""
ms.date: "11/04/2016"
ms.reviewer: ""
ms.suite: ""
...
>  Visual Studio 2015 では、式エバリュエーターを実装するには、この方法は推奨されません。 CLR 式エバリュエーターの実装については、次を参照してください [CLR 式エバリュエーター](https://github.com/Microsoft/ConcordExtensibilitySamples/wiki/CLR-Expression-Evaluators) と [マネージ式エバリュエーターのサンプル](https://github.com/Microsoft/ConcordExtensibilitySamples/wiki/Managed-Expression-Evaluator-Sample)します。  
 ...
 表示されるすべてのローカル、 **\[ローカル\]** ウィンドウには、 [IDebugProperty2](../../extensibility/debugger/reference/idebugproperty2.md) オブジェクトが関連付けられています。 これは、 `IDebugProperty2` オブジェクトには、ローカルの名前、値、および種類が含まれています。 Visual Studio を呼び出すユーザーは、ローカルの値を変更するときに [SetValueAsString](../../extensibility/debugger/reference/idebugproperty2-setvalueasstring.md) をメモリにローカル変数の値を更新します。 この例では、ローカルがによって表される、 `CFieldProperty` を実装するクラス、 `IDebugProperty2` インターフェイスです。  
  
> [!NOTE]
>  **ウォッチ** と **\[クイック ウォッチ\]** 式、値が変更されたはによって表される、 `CValueProperty` MyCEE サンプル内のクラスです。 ただしの実装 `IDebugProperty2::SetValueAsString` 、次のように同じです。  
  
 この実装の `IDebugProperty2::SetValueAsString` 次のタスクを実行します。  
  
1.  値を生成する式を評価します。  
  
2.  関連付けられたバインド [IDebugField](../../extensibility/debugger/reference/idebugfield.md) メモリ位置にオブジェクトを作成、 [IDebugObject](../../extensibility/debugger/reference/idebugobject.md) オブジェクトです。  
  
3.  一連のバイト値に変換します。  
  
4.  呼び出し [値の代入](../Topic/IDebugObject::SetValue.md) でメモリにバイトを格納します。  
  
## マネージ コード  
 これは、実装の `IDebugProperty2::SetValueAsString` マネージ コードにします。  
...
## アンマネージ コード  
 これは、実装の `IDebugProperty2::SetValueAsString` マネージ コードにします。 ヘルパー関数 `FieldCoerceValueType` \(表示されていません\) の力、 `VARIANT` 値では、特定の種類を加えますことを確認するが、型の 1 つ `FieldSetValue` を処理できます。  
