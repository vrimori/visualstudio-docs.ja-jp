...
このブレークポイントの要求を記述したブレークポイントの要求された情報を取得します。  
...
#### パラメーター  
 `dwFields`  
 \[入力\] `pBPRequestInfo` のパラメーターのフィールドが設定する方法を決定 [BPREQI\_FIELDS](../../../extensibility/debugger/reference/bpreqi-fields.md) の列挙体のフラグの組み合わせ。  
  
 `pBPRequestInfo`  
 \[出力\] ブレークポイントの要求の記述が格納される [BP\_REQUEST\_INFO](../../../extensibility/debugger/reference/bp-request-info.md) の構造体を指定します。  
