...
3.  ブレークポイントを使用してまたはそれ以外の場合、VSPackage が読み込まれ、SetItemAttribute が実行されています。  
    ...
    > [!NOTE]
    >  Autoload UI コンテキストで VSPackage を作成することができます<xref:Microsoft.VisualStudio.VSConstants.UICONTEXT_SolutionExists>です。 詳細については、次を参照してください。[読み込み Vspackage](../extensibility/loading-vspackages.md)です。  
