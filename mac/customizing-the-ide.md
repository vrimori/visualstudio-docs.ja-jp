...
Visual Studio for Mac ではテーマを切り替えることができます。**[Visual Studio]、[ユーザー設定...]、[環境]、[視覚スタイル]** の順に選択し、**[ユーザー インターフェイス テーマ]** ドロップダウンから希望のテーマを選択します。下のスクリーンショットをご覧ください。

 ![ダーク テーマの選択](media/customizing-the-ide-image7b.png)

## <a name="localization"></a>ローカリゼーション

Visual Studio for Mac は 13 カ国語にローカライズされており、よりたくさんの開発者にご利用いただけます。 現在のところ、以下の言語でご利用いただけます。
...
Visual Studio for Mac の表示言語を変更するには、**[Visual Studio]、[ユーザー設定...]、[環境]、[視覚スタイル]** の順に選択し、**[ユーザー インターフェイス言語]** ドロップダウンから希望の言語を選択します。下のスクリーンショットをご覧ください。
...
作成者情報パネルでは、下の画像のように、名前、メール アドレス、作品の著作権所有者、会社、商標など、自分に関する関連情報を追加できます。

 ![作成者情報の編集セクション](media/customizing-the-ide-image9a.png)

この情報は、Visual Studio for Mac で作成した新しいファイルに追加する可能性がある、ライセンスなど、標準ファイル ヘッダーに入力するために使用されます。

 ![標準ヘッダー オプション](media/customizing-the-ide-image8a.png)


Visual Studio for Mac のバージョン管理からコミットが行われた場合、そのコミットに情報を追加するために、**[名前]** フィールドと **[電子メール]** フィールドに入力されたデータが利用されます。 このフィールドにデータを入力していない場合、バージョン管理の利用時、Visual Studio for Mac から入力が求められます。
...
下の画像のように、**[Visual Studio]、[ユーザー設定...]、[環境]、[キー バインド]** の順に選択し、キー バインドを設定できます。
...
Visual Studio for Mac のワークスペースは、メインのドキュメント領域 (通常、エディター、デザイナー サーフェス、またはオプション ファイル) で構成されています。この補完的な*領域*はパッドで囲まれています。このパッドには、アプリケーション ファイルにアクセスし、管理、テスト、およびデバッグを行う際に役立つ情報が含まれています。

 ![ワークスペースのレイアウト](media/customizing-the-ide-image1a.png)

### <a name="viewing-and-arranging-pads"></a>パッドの表示と整理

Visual Studio for Mac で新しいソリューションまたはファイルを開くとき、ワークスペースに*パッド*が表示されます。これには、下の画像のように、Solution Pad、ドキュメント アウトライン、エラーが含まれます。
...
開いているパッドの状態を表現したものが*レイアウト*です。 下の画像のように、レイアウトは [表示] メニューから手動で切り替えることができます。あるいは、デバッグしたり、ストーリーボードを開いたり、特定のアクションを実行すると自動的に切り替わります。
...
2 列モードは [表示] メニュー項目から有効にできます。**[表示]、[エディター列]、[2 列]** の順に選択するか、エディター領域の 1 つの端にエディター タブをドラッグします。下の画像をご覧ください。
